...
engine = create_engine(
    "sqlite:///films.db", connect_args={"check_same_thread": False}
)
...
def gconnect():
    ...
    if result['user_id'] != gplus_id:
        response = make_response(
            json.dumps("Token's user ID doesn't match app's."), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def catalogueJSON():
    ...
    return jsonify(
        genres=[g.serialize for g in genres],
        films=[f.serialize for f in films],
    )
...
# JSON APIs to view Genre Information of a specific one & its respective films
...
def showGenres():
    ...
    if 'username' not in login_session:
        return render_template(
            'publicgenremenu.html', genres=genres, STATE=state)
    else:
        return render_template(
            'genremenu.html', genres=genres, STATE=state)
...
def editGenre(genre_id):
    ...
    if genreToEdit.user_id != login_session["user_id"]:
        ...
        output += " to edit this film genre. Please create your own genre in"
        output += " order to edit.');}</script><body onload='myFunction()''>"
        return output
    if request.method == "POST":
...
def deleteGenre(genre_id):
    ...
    if genreToDelete.user_id != login_session["user_id"]:
        output = ""
        output += "<script>function myFunction(){alert('You are not authorized"
        output += " to delete this genre. Please create your own film genre in"
...
def newFilm(genre_id):
    ...
    if genre.user_id != login_session["user_id"]:
        output = ""
        output += "<script>function myFunction(){alert('You are not authorized"
        output += " to create a film. Please create your own genre in"
        output += " order to do so.');}</script><body onload='myFunction()''>"
        return output
    if request.method == "POST":
        newFilm = Film(
...
# Edit a film
@app.route(
    "/genre/<int:genre_id>/film/<int:film_id>/edit", methods=["GET", "POST"]
)
def editFilm(genre_id, film_id):
    ...
    if genre.user_id != login_session["user_id"]:
        output = ""
        output += "<script>function myFunction(){alert('You are not authorized"
        output += " to edit this film. Please create your own film genre in"
        output += " order to edit.');}</script><body onload='myFunction()''>"
        return output
    if request.method == "POST":
...
# Delete a film
@app.route(
    "/genre/<int:genre_id>/film/<int:film_id>/delete", methods=["GET", "POST"]
)
def deleteFilm(genre_id, film_id):
    ...
    if genre.user_id != login_session["user_id"]:
        output = ""
        output += "<script>function myFunction(){alert('You are not authorized"
        output += " to delete this film. Please create your own film genre in"
        output += " order to do so.');}</script><body onload='myFunction()''>"
        return output
    if request.method == "POST":
